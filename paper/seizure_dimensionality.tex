...
\subsection{Effective Dimensionality as a Window into Criticality}

We operationalize ``coordination'' through the eigenvalue spectrum of the EEG channel covariance matrix. The participation ratio $\Dpr$ measures how many independent modes contribute to the signal:
\begin{equation}
\Dpr = \frac{\left(\sum_i \lambda_i\right)^2}{\sum_i \lambda_i^2}
\end{equation}

Counterintuitively, \textit{higher} $\Dpr$ can indicate \textit{more} coordination---when previously independent oscillators begin to synchronize, they form coherent modes that become visible in the covariance structure. The key insight is that we must track not just $\Dpr$ itself, but its \textbf{variance over time}:
\begin{itemize}
...
\subsection{The Paradox: Pre-ictal Shows Higher, Not Lower, Dimensionality}
...
All comparisons between interictal and pre-ictal are statistically significant ($p < 0.01$) given the large sample sizes, but the pattern is what matters:
\begin{itemize}
...
Logistic regression classification of pre-ictal vs.\ interictal using dimensional features achieved:
\begin{itemize}
    \item AUC = $0.68 \pm 0.05$ (5-fold CV)
\end{itemize}

This moderate performance reflects the challenge: pre-ictal is not ``obviously abnormal'' in simple metrics. The discriminative information is in the \textit{variance reduction}, which requires tracking metrics over time rather than classifying individual windows.
...
All analysis code is available at: \url{https://github.com/[repository]}
