...
\textbf{The orchestra analogy.} Consider an orchestra with 100 musicians. How many ``dimensions'' of activity are there? It depends on what they're playing:

\begin{itemize}
    \item \textbf{Unison passage}: All 100 musicians play the same note. Despite 100 instruments, there is effectively \textit{one} dimension---one pattern of activity replicated across many sources.

    \item \textbf{Four-part fugue}: The orchestra splits into soprano, alto, tenor, bass voices. Now there are \textit{four} independent patterns, even though 100 musicians are still playing.

    \item \textbf{Warm-up cacophony}: Each musician plays independently---scales, fragments, tuning. This is \textit{high-dimensional}: many independent patterns coexisting.
\end{itemize}

The key insight is that dimensionality measures \textit{independence}, not quantity. We hire 100 musicians precisely \textit{because} we want the capacity for rich, varied sound---not to have them all do the same thing. Each violin can produce different timbres; each section can carry its own melodic line; the full orchestra can generate complex harmonies impossible for a single instrument.

\textbf{Why high dimensionality is healthy.} A healthy brain, like a functioning orchestra, maintains high effective dimensionality: different regions can do different things, respond to different inputs, carry different information. The visual cortex processes images while the motor cortex plans movement while the prefrontal cortex maintains goals. This \textit{functional diversity} requires high dimensionality.

\textbf{When dimensionality collapses.} But sometimes an orchestra locks into unison---a dramatic climax where all instruments hammer the same chord. This is low-dimensional: powerful, but inflexible. The brain equivalent is a seizure: all neurons firing together in hypersynchronous discharge. The rich, varied activity collapses into a single overwhelming pattern.

\textbf{The math.} We formalize this by computing the covariance matrix of the EEG channels and decomposing it into eigenvalues $\lambda_1 \geq \lambda_2 \geq \ldots \geq \lambda_n$. Each eigenvalue represents the variance explained by one spatial pattern (eigenvector):

\begin{itemize}
    \item If all variance concentrates in one mode ($\lambda_1$ dominates), the signal is effectively one-dimensional---all channels are doing the same thing (unison).
    \item If variance spreads across many modes (many $\lambda_i$ are similar), the signal is high-dimensional---multiple independent patterns coexist (fugue or cacophony).
\end{itemize}

The \textbf{participation ratio} $\Dpr$ summarizes this in a single number:
\begin{equation}
\Dpr = \frac{\left(\sum_i \lambda_i\right)^2}{\sum_i \lambda_i^2}
\end{equation}
This equals $n$ (maximum) if all eigenvalues are equal, and approaches 1 (minimum) if one eigenvalue dominates. For 23-channel EEG, $\Dpr$ typically ranges from 3--10 depending on brain state.
...
For each 10-second window, we asked: ``How is the variance distributed across spatial patterns?'' This is analogous to asking an orchestra: ``How many independent voices are active right now?''

\textbf{Step 1: Compute covariance.} We computed the channel covariance matrix $\mathbf{C} = \mathbf{X}^\top \mathbf{X} / (n-1)$ where $\mathbf{X}$ is the $T \times N$ matrix of filtered EEG (T timepoints, N = 23 channels). The covariance matrix captures which channels tend to fluctuate together.

\textbf{Step 2: Find the eigenvalues.} We decomposed $\mathbf{C}$ into eigenvalues $\lambda_1 \geq \lambda_2 \geq \cdots \geq \lambda_N$. Each eigenvalue represents the variance captured by one independent spatial pattern. Large $\lambda_1$ means most variance is in one pattern (the orchestra playing in unison); more equal eigenvalues mean multiple independent patterns (the fugue).

\textbf{Step 3: Summarize the spectrum.} We computed four complementary metrics:

\subsubsection{Participation Ratio ($\Dpr$)}
\begin{equation}
\Dpr = \frac{\left(\sum_i \lambda_i\right)^2}{\sum_i \lambda_i^2}
\end{equation}
\textit{Intuition:} ``How many modes are really contributing?'' If one eigenvalue dominates, $\Dpr \approx 1$. If all are equal, $\Dpr = N$. For our 23-channel EEG, typical values range from 3 (very synchronized) to 10 (diverse activity).
...
\end{equation}
\textit{Intuition:} ``How many modes do we need to capture most of the action?'' $\Kth{95} = 10$ means the top 10 modes explain 95\% of variance. Lower $\Kth{\theta}$ indicates more concentrated (lower-dimensional) activity.

\subsubsection{Spectral Slope ($\beta$)}
We fit $\log \lambda_i = \alpha - \beta \log i$.

\textit{Intuition:} ``How steeply does the spectrum fall off?'' More negative $\beta$ means variance is more concentrated in the top modes. A steep slope ($\beta = -3$) indicates dominance by few patterns; a shallow slope ($\beta = -2$) indicates more distributed activity.

\subsubsection{Dominant Mode Fraction ($\lambda_1$)}
\begin{equation}
f_1 = \lambda_1 / \sum_i \lambda_i
\end{equation}
\textit{Intuition:} ``What fraction of the signal is the single loudest pattern?'' High $f_1$ (e.g., 0.5) means half the variance is in one mode---the orchestra is mostly playing one thing. Low $f_1$ (e.g., 0.2) means the dominant pattern is just one voice among several.
...
\textbf{Cardiac dimensionality and HRV:} Our ``brain rate variability'' concept draws on the HRV literature, but the analogy may be deeper than metaphor. The heart, like the brain, contains millions of semi-independent oscillators: each cardiomyocyte has its own intrinsic pacemaker capacity, normally entrained by the sinoatrial node but retaining independence. Healthy cardiac function may require \textit{high effective dimensionality}---not all cells locked in perfect synchrony, but a flexible coordination that allows beat-to-beat adaptation.

Traditional HRV analysis measures temporal variability in a scalar (R-R interval). Multi-lead ECG, however, provides spatial information analogous to multi-channel EEG. We hypothesize that:
\begin{itemize}
    \item \textbf{Healthy heart}: Moderate spatial dimensionality in the ECG, high temporal variance---flexible, adaptive.
    \item \textbf{Heart failure / fixed rate}: Low dimensionality, low variance---rigid, fragile.
    \item \textbf{Arrhythmia / V-fib}: High dimensionality, high variance---chaotic, uncoordinated.
\end{itemize}
This would parallel our seizure findings: pathology as departure from optimal dimensionality in either direction. Applying identical eigenvalue-based analysis to multi-lead ECG databases (e.g., PTB-XL) could test whether ``dimensional health'' is a general principle across excitable tissues.
